...
SELECTS: tuple[GuideVaultSelectDescription, ...] = (
    GuideVaultSelectDescription(
        ...
        value_keys=("background", "reader.background", "backgroundName", "reader.backgroundName"),
        options_keys=("backgrounds", "availableBackgrounds", "reader.availableBackgrounds", "settings.backgrounds"),
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await async_send_command(
            ...
            self._entry.entry_id,
            {
                ...
                self._description.payload_key: option,
            },
        )
...
def _coerce_options(value: Any) -> list[str]:
    ...
    if isinstance(value, list):
        ...
        for item in value:
            if isinstance(item, dict):
                name = item.get("value") or item.get("id") or item.get("name") or item.get("label")
                if name not in (None, ""):
    ...
    if isinstance(value, dict):
        return _dedupe(str(key) for key in value.keys())
